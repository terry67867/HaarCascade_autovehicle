...
int main()
{
    ...
    if (!car_cascade.load("/home/terry67867/car_cascade.xml")) { 
        std::cout << "Error when loading the cascade classfier!" << std::endl; 
        return -1; 
    }
    cv::CascadeClassifier bike_cascade; //Bike
    if (!car_cascade.load("/home/terry67867/opencv/Development/Product/bike_cascade.xml")) { 
        std::cout << "Error when loading the cascade classfier!" << std::endl; 
        return -1; 
    }
    ...
    //영상 읽기 
    cap.open("/home/terry67867/Product/RightMode02.mp4");
